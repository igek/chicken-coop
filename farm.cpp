...
int main (int argc, char const * argv[]) {
  ...
  while (cash_difference > 0) {
    ...
    // Lay eggs
	if (diff_chicken >= std::chrono::milliseconds(6)) {
    ...
    // Hatch eggs
	if (diff_egg >= std::chrono::milliseconds(96)) {
	...
	if (diff_sell >= std::chrono::milliseconds(21)) {
    ...
    // Pay Upkeep
	if (diff_upkeep >= std::chrono::milliseconds(696)) {
		float cash = 0;
